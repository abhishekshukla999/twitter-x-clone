...
function NavigationBottom() {
    const listStyle =
        "flex p-3 text-xl hover:bg-zinc-200 dark:hover:bg-slate-500 dim:hover:bg-slate-500 w-fit";

    return (
        <div className="nav-bottom hidden max-[499px]:flex justify-between fixed bottom-0 bg-white dark:bg-twitter-lightsout-bg dim:bg-twitter-dim-bg w-full">
            <div>
                <NavLink
                    to="/home"
                    className={({ isActive }) =>
                        `${listStyle} ${
                            isActive
                                ? "bg-gray-200 dark:bg-slate-800 dim:bg-slate-700"
                                : null
                        }`
                    }
                    title="Home"
